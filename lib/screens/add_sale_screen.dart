...
class _AddSaleScreenState extends State<AddSaleScreen> {
  ...
  void _addSaleItem(Product selectedProduct, int quantity) {
    ...
    if (selectedProduct.stockQuantity < quantity) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content:
              Text('Not enough stock. Available: ${selectedProduct.stockQuantity}')));
      return;
    }

    int existingIndex = _currentSaleItems
        .indexWhere((item) => item.productId == selectedProduct.productId);
    ...
    if (existingIndex != -1) {
      ...
      if (selectedProduct.stockQuantity <
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Exceeds available stock.')));
        return;
      }

      setState(() {
        _currentSaleItems[existingIndex] =
            ...
            child: Column(
              ...
              children: [
                ...
                FormBuilderTextField(
                    name: 'last_name',
                    decoration:
                        _fieldDecoration('Last Name', Icons.person_outline),
                    validator: (val) =>
                        val == null || val.isEmpty ? 'Required' : null),
                const SizedBox(height: 12),
                FormBuilderTextField(
                ...
                FormBuilderTextField(
                    name: 'phone',
                    decoration: _fieldDecoration('Phone', Icons.phone),
  ...
  // ---------------------------------------------------------------------------
  // POPUP PRODUCT SELECTOR (GRID + SEARCH)
  // ---------------------------------------------------------------------------
  ...
  void _showProductSelectionPopup(
    ...
    showDialog(
      context: context,
      builder: (popupCtx) {
        return StatefulBuilder(builder: (popupCtx, setPopupState) {
          final filteredProducts = productProvider.products.where((p) {
            final search = _productSearchController.text.toLowerCase();
            return p.product.status == "Available" &&
                p.product.stockQuantity > 0 &&
                p.product.productName.toLowerCase().contains(search);
          }).toList();
          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: GridView.builder(
                ...
                itemBuilder: (ctx, i) {
                  final prod = filteredProducts[i].product;
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: FormBuilder(
          ...
          enabled:
              !customerProvider.isLoading && !productProvider.isLoading,
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildSectionHeader('Transaction Details'),
              _buildTransactionCard(context, customerProvider),
              ...
              _buildSummaryCard(context, productProvider),

              const SizedBox(height: 32),

              _buildSubmitButton(context, isEditing, productProvider),
            ],
          ),
        ),
      ),
  ...
  Widget _buildTransactionCard(
      BuildContext context, CustomerProvider customerProvider) {
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(
              ...
              children: [
                ...
                Expanded(
                  child: FormBuilderDropdown<String>(
                    name: 'payment_method',
                    decoration:
                        _fieldDecoration('Method', Icons.payment),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            GestureDetector(
              onTap: () =>
                  _showProductSelectionPopup(context, productProvider),
              child: AbsorbPointer(
                child: TextFormField(
                  decoration: _fieldDecoration(
                      'Select Product', Icons.inventory_2,
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration:
                        _fieldDecoration('Quantity', Icons.onetwothree),
      ...
      child: ElevatedButton(
        onPressed: () async {
          if (_formKey.currentState?.saveAndValidate() ?? false) {
            ...
            final finalItems = _currentSaleItems
            ...
            try {
              if (isEditing) {
                await Provider.of<SaleProvider>(context, listen: false)
                    .updateSale(newSale, finalItems);
              } else {
                await Provider.of<SaleProvider>(context, listen: false)
                    .addSale(newSale, finalItems);
              }
        ...
        child: Text(isEditing ? 'Update Sale' : 'Complete Sale',
            style: const TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: Colors.white)),
